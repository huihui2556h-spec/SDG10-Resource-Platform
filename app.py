...
# --- 🆕 SQL 資料庫連線初始化 ---
# Streamlit 會自動去尋找 .streamlit/secrets.toml 裡的設定
try:
    conn = st.connection("mysql", type="sql")
except Exception as e:
    st.error(f"❌ 無法連線到資料庫，請檢查 secrets.toml 設定。錯誤訊息: {e}")
    ...
    if not df.empty:
        ...
        with col_out:
            if "matched_list" in st.session_state and st.session_state.matched_list is not None:
                ...
                feedback_msg = st.text_area("優化建议：")
                    ...
                    if 'conn' in globals():
                        try:
                            with conn.session as session:
                                # 將資料寫入 XAMPP MySQL
                                session.execute(
                                    "INSERT INTO user_feedback (category, score, message) VALUES (:category, :score, :message);",
                                    {"category": u_need, "score": feedback_score, "message": feedback_msg}
                                )
                                session.commit()
                            st.success("🎉 回饋已成功安全地儲存至 SQL 資料庫！")
                        except Exception as e:
                            st.error(f"資料庫儲存失敗，請檢查資料表是否存在。錯誤: {e}")
                    else:
                        st.error("未成功建立資料庫連線，無法儲存。")
            else:
...
# --- 5. 頁面 C：管理員數據中心 ---
    ...
    if pwd == "1234": # 密碼 1234
        ...
        if 'conn' in globals():
            try:
                # 從 MySQL 資料庫抓取所有回饋，並依時間倒序排列
                display_df = conn.query("SELECT category AS 類別, score AS 評分, message AS 建議, created_at AS 提交時間 FROM user_feedback ORDER BY created_at DESC;")
                
                if not display_df.empty:
                    st.write("### 📥 使用者測試回饋紀錄 (來自 SQL)")
                    st.dataframe(display_df, use_container_width=True)
                    
                    csv_data = display_df.to_csv(index=False).encode('utf-8-sig')
                    st.download_button(label="📥 下載完整報告 (.csv)", data=csv_data, file_name="admin_report.csv")
                else:
                    st.info("目前資料庫內尚無回饋紀錄。")
            except Exception as e:
                st.error(f"無法從資料庫讀取資料。錯誤: {e}")
        else:
            st.error("資料庫連線未建立。")
